...
var methods = {
  prepCompo: function(collector) {
    collector.on("collect", message => {
      ...
      if (splitemess[0] === "+") {
        ...
        message.channel.send(
          "__Composition actuelle de la partie__ : " +
            datas.compo[message.guild.id]
        );
        ...
        if (
          ...
          datas.inscrits[message.guild.id].length
        ) {
          var diffSV =
            datas.inscrits[message.guild.id].length -
            datas.nbRole[message.guild.id];
          ...
          collector11.on("collect", message => {
            ...
            if (message.content === "oui" || message.content === "o") {
              ...
              message.channel.send(
                "Il reste " + diffSV + " rôles à attribuer."
              );
              collector11.stop();
            }
          });
  ...
  annonceCompo: function(message) {
    ...
    datas.compo[message.guild.id].forEach(role => {
      embed.addField(role.Quantite + " " + role.Name, role.Emote);
    });
    ...
    collector22.on("collect", message => {
      ...
      if (message.content === "send") {
        ...
        var lieu = dbutils.getPlaceInDb("village", message);
      ...
      datas.compo[message.guild.id].forEach(Role => {
        if (Role.Name === role) {
          ...
          try {
            ...
            message.channel.send(
              `Erreur : le rôle ${role} n'a pas été trouvé dans la liste. Merci de reporter cette erreur aux administrateurs. Détail : **${e.message}**`
            );
          }
        } else {
          i++;
      ...
      if (Role.Name === role) {
        ...
        if (item.Quantite <= 0) {
          datas.compo[message.guild.id].splice(
            datas.compo[message.guild.id].indexOf(item),
            1
          );
        }
      }
