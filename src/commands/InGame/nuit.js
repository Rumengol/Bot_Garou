...
execute(client, message, args){
  ...
  if (
    ...
    datas.rolesDeNuit[message.guild.id].splice(
      datas.rolesDeNuit[message.guild.id].indexOf(datas.Soso[message.guild.id]),
      1
    );
  }
    ...
    message => {
      ...
      //Action nocture de la Sorcière
        ...
        if (spliteMessage.length != 2) {
          ...
          rolajouer.splice(rolajouer.indexOf(datas.Soso[message.guild.id]), 1);
                ...
                collectorSoso.on("collect", reac => {
                  if (
                    ...
                    if (reac.emoji.name === "❌") {
                      ...
                      datas.salonLog[message.guild.id].send(
                        datas.Soso[message.guild.id] + " n'a protégé personne."
                      );
      ...
      //Action nocturne du Joueur de flûte
        ...
        if (acharme[0] === undefined) {
          for (
            ...
            acharme.push(datas.distribRoles[message.guild.id][i].User.username);
          }
        }
